...
import (
	...
	"github.com/freight-trust/zeroxyz/internal/besukafka"
	"github.com/freight-trust/zeroxyz/internal/besurest"
	"github.com/freight-trust/zeroxyz/internal/besuutils"
	log "github.com/sirupsen/logrus"
	"github.com/spf13/cobra"
	prefixed "github.com/x-cray/logrus-prefixed-formatter"
)

// ServerConfig is the parent YAML structure that configures zeroxyz
...
type ServerConfig struct {
	KafkaBridges map[string]*besukafka.KafkaBridgeConf `json:"kafka"`
	Webhooks     map[string]*besurest.RESTGatewayConf  `json:"webhooks"`
	RESTGateways map[string]*besurest.RESTGatewayConf  `json:"rest"`
	...
	serverCmd = &cobra.Command{
		...
		PreRunE: func(cmd *cobra.Command, args []string) (err error) {
			if serverCmdConfig.Filename == "" {
				err = besuerrors.Errorf(besuerrors.ConfigNoYAML)
				return
			}
			return
		},
...
func readServerConfig() (serverConfig *ServerConfig, err error) {
	...
	if err != nil {
		err = besuerrors.Errorf(besuerrors.ConfigFileReadFailed, serverCmdConfig.Filename, err)
		return
	}
	if strings.ToLower(serverCmdConfig.Type) == "yaml" {
		...
		if err = yaml.Unmarshal(confBytes, &yamlGenericPayload); err != nil {
			err = besuerrors.Errorf(besuerrors.ConfigYAMLParseFile, serverCmdConfig.Filename, err)
			return
		}
	...
	if err != nil {
		err = besuerrors.Errorf(besuerrors.ConfigYAMLPostParseFile, serverCmdConfig.Filename, err)
		return
	}
...
func startServer() (err error) {
	...
	if rootConfig.PrintYAML {
		b, err := besuutils.MarshalToYAML(&serverConfig)
	...
	for name, conf := range serverConfig.KafkaBridges {
		kafkaBridge := besukafka.NewKafkaBridge(&dontPrintYaml)
	...
	if serverConfig.RESTGateways == nil {
		serverConfig.RESTGateways = make(map[string]*besurest.RESTGatewayConf)
	}
	...
	for name, conf := range serverConfig.RESTGateways {
		restGateway := besurest.NewRESTGateway(&dontPrintYaml)
...
func init() {
	...
	kafkaBridge := besukafka.NewKafkaBridge(&rootConfig.PrintYAML)
	rootCmd.AddCommand(kafkaBridge.CobraInit())

	restGateway := besurest.NewRESTGateway(&rootConfig.PrintYAML)
