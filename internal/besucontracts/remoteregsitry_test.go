...
func (rr *mockRR) loadFactoryForGateway(id string, refresh bool) (*maidenlanedmessages.DeployContract, error) {
	rr.idCapture = id
...
func TestRemoteRegistryloadFactoryForGatewaySuccess(t *testing.T) {
	...
	runtimeABI, err := maidenlanedbind.ABIMarshalingToABIRuntime(res.ABI)
	assert.NoError(err)
