...
func NewContractDeployTxn(msg *besudmessages.DeployContract, signer TXSigner) (tx *Txn, err error) {
	...
	if msg.Compiled != nil && msg.ABI != nil {
		...
		err = besuderrors.Errorf(besuderrors.DeployTransactionMissingCode)
		return
	}

	// Build a runtime ABI from the serialized one
	var typedArgs []interface{}
	abi, err := besudbind.ABIMarshalingToABIRuntime(compiled.ABI)
	if err == nil {
	...
	if err != nil {
		err = besuderrors.Errorf(besuderrors.TransactionSendConstructorPackArgs, err)
		return
	}
...
func CallMethod(ctx context.Context, rpc RPCClient, signer TXSigner, from, addr string, value json.Number, methodABI *besudbind.ABIMethod, msgParams []interface{}, blocknumber string) (map[string]interface{}, error) {
	...
	// "latest" and "" (no besud-blocknumber given) are equivalent
	if blocknumber != "" && blocknumber != "latest" {
		...
		if isHex || blocknumber == "earliest" || blocknumber == "pending" {
			...
			if !ok {
				return nil, besuderrors.Errorf(besuderrors.TransactionCallInvalidBlockNumber)
			}
...
func ProcessRLPBytes(args abi.Arguments, retBytes []byte) map[string]interface{} {
	...
	var err error
	if unpackErr != nil {
		err = besuderrors.Errorf(besuderrors.UnpackOutputsFailed, unpackErr)
	} else {
...
func processOutputs(args abi.Arguments, rawRetval []interface{}, retval map[string]interface{}) error {
	...
	if numOutputs > 0 {
		if len(rawRetval) != numOutputs {
			return besuderrors.Errorf(besuderrors.UnpackOutputsMismatchCount, numOutputs, len(rawRetval), rawRetval)
		}
		...
		return besuderrors.Errorf(besuderrors.UnpackOutputsMismatchNil, rawRetval)
	}
	return nil
}
...
func mapOutput(argName, argType string, t *abi.Type, rawValue interface{}) (interface{}, error) {
	...
	case abi.IntTy, abi.UintTy:
		...
		if kind == reflect.Ptr && rawType.String() == "*big.Int" {
			...
			return nil, besuderrors.Errorf(besuderrors.UnpackOutputsMismatchType, "number",
				argName, argType, rawType.Kind())
		}
	case abi.BoolTy:
		if rawType.Kind() != reflect.Bool {
			return nil, besuderrors.Errorf(besuderrors.UnpackOutputsMismatchType, "boolean",
				argName, argType, rawType.Kind())
		}
		return rawValue, nil
	case abi.StringTy:
		if rawType.Kind() != reflect.String {
			return nil, besuderrors.Errorf(besuderrors.UnpackOutputsMismatchType, "string array",
				argName, argType, rawType.Kind())
		}
	...
	case abi.BytesTy, abi.FixedBytesTy, abi.AddressTy:
		if (rawType.Kind() != reflect.Array && rawType.Kind() != reflect.Slice) || rawType.Elem().Kind() != reflect.Uint8 {
			return nil, besuderrors.Errorf(besuderrors.UnpackOutputsMismatchType, "[]byte",
				argName, argType, rawType.Kind())
		}
	...
	case abi.SliceTy, abi.ArrayTy:
		if rawType.Kind() != reflect.Slice {
			return nil, besuderrors.Errorf(besuderrors.UnpackOutputsMismatchType, "slice",
				argName, argType, rawType.Kind())
		}
		s := reflect.ValueOf(rawValue)
	...
	default:
		return nil, besuderrors.Errorf(besuderrors.UnpackOutputsUnknownType,
			argName, argType, rawType.Kind())
	}
}
...
func genTupleMapOutput(argName, argType string, t *abi.Type, rawValue interface{}) (r map[string]interface{}, err error) {
	...
	if reflectValue.Kind() != reflect.Struct || reflectValue.Type() != t.TupleType {
		return nil, besuderrors.Errorf(besuderrors.UnpackOutputsMismatchTupleType,
			argName, argType, t.TupleType, rawValue)
	}
	if len(t.TupleRawNames) != reflectValue.NumField() {
		return nil, besuderrors.Errorf(besuderrors.UnpackOutputsMismatchTupleFieldCount,
...
// SendTranasction message
func NewSendTxn(msg *besudmessages.SendTransaction, signer TXSigner) (tx *Txn, err error) {

	var methodABI *abi.Method
	if msg.Method == nil || msg.Method.Name == "" {
		if msg.MethodName == "" {
			err = besuderrors.Errorf(besuderrors.TransactionSendMissingMethod)
			return
		}
		...
		methodABI, err = besudbind.ABIElementMarshalingToABIMethod(msg.Method)
		if err != nil {
			return
		}
	}
...
func buildTX(signer TXSigner, msgFrom, msgTo string, msgNonce, msgValue, msgGas, msgGasPrice json.Number, methodABI *abi.Method, params []interface{}) (tx *Txn, err error) {
	...
	if err != nil {
		err = besuderrors.Errorf(besuderrors.TransactionSendMethodPackArgs, methodABI.RawName, err)
...
func (tx *Txn) genEthTransaction(msgFrom, msgTo string, msgNonce, msgValue, msgGas, msgGasPrice json.Number, data []byte) (err error) {
	...
	if msgFrom != "" {
		tx.From, err = besudutils.StrToAddress("from", msgFrom)
		if err != nil {
			return
		}
	}
	...
	if msgValue.String() != "" {
		if _, ok := value.SetString(msgValue.String(), 10); !ok {
			err = besuderrors.Errorf(besuderrors.TransactionSendBadValue, err)
			return
		}
	}
	...
	if msgGas != "" {
		...
		if err != nil {
			err = besuderrors.Errorf(besuderrors.TransactionSendBadGas, err)
			return
		}
	}
	...
	if msgGasPrice.String() != "" {
		if _, ok := gasPrice.SetString(msgGasPrice.String(), 10); !ok {
			err = besuderrors.Errorf(besuderrors.TransactionSendBadGasPrice)
			return
		}
	}
	...
	var toStr string
	if msgTo != "" {
		if toAddr, err = besudutils.StrToAddress("to", msgTo); err != nil {
			return
		}
...
func (tx *Txn) getInteger(methodName string, path string, requiredType *abi.Type, suppliedType reflect.Type, param interface{}) (val int64, err error) {
	if suppliedType.Kind() == reflect.String {
		if val, err = strconv.ParseInt(param.(string), 10, 64); err != nil {
			err = besuderrors.Errorf(besuderrors.TransactionSendInputTypeBadNumber, methodName, path)
			return
		}
	} else if suppliedType.Kind() == reflect.Float64 {
		val = int64(param.(float64))
	} else {
		err = besuderrors.Errorf(besuderrors.TransactionSendInputTypeBadJSONTypeForNumber, methodName, path, requiredType, suppliedType)
	}
	return
}
...
func (tx *Txn) generateTypedArrayOrSlice(methodName string, path string, requiredType *abi.Type, suppliedType reflect.Type, param interface{}) (interface{}, error) {
	if suppliedType.Kind() != reflect.Slice {
		return nil, besuderrors.Errorf(besuderrors.TransactionSendInputTypeBadJSONTypeForArray, methodName, path, requiredType, suppliedType)
	}
...
func (tx *Txn) generateTupleFromMap(methodName string, path string, requiredType *abi.Type, param map[string]interface{}) (v interface{}, err error) {
	...
	for i, inputElemName := range requiredType.TupleRawNames {
		...
		if suppliedType == nil {
			...
			return nil, besuderrors.Errorf(besuderrors.TransactionSendInputNotAssignable, methodName, path, typedVal, inputElemName, requiredType.TupleElems[i])
		}
...
func (tx *Txn) generateTypedArg(requiredType *abi.Type, param interface{}, methodName string, path string) (interface{}, error) {
	...
	if suppliedType == nil {
		return nil, besuderrors.Errorf(besuderrors.TransactionSendInputTypeBadNull, methodName, path)
	}
	...
	case abi.BoolTy:
		...
		return nil, besuderrors.Errorf(besuderrors.TransactionSendInputTypeBadJSONTypeForBoolean, methodName, path, requiredType, suppliedType)
	case abi.StringTy:
		if suppliedType.Kind() == reflect.String {
			return param.(string), nil
		}
		return nil, besuderrors.Errorf(besuderrors.TransactionSendInputTypeBadJSONTypeForString, methodName, path, suppliedType)
	case abi.AddressTy:
		if suppliedType.Kind() == reflect.String {
			if !common.IsHexAddress(param.(string)) {
				return nil, besuderrors.Errorf(besuderrors.TransactionSendInputTypeAddress, methodName, path, suppliedType)
			}
			return common.HexToAddress(param.(string)), nil
		}
		return nil, besuderrors.Errorf(besuderrors.TransactionSendInputTypeBadJSONTypeForAddress, methodName, path, requiredType, suppliedType)
		...
		if suppliedType.Kind() == reflect.Slice {
			...
			for i := 0; i < bSliceLen; i++ {
				...
				if valV.Kind() != reflect.Float64 {
					return nil, besuderrors.Errorf(besuderrors.TransactionSendInputTypeBadJSONTypeInNumericArray, methodName, path, requiredType, i, valV.Kind())
				}
				floatVal := valV.Float()
				if floatVal > 255 || floatVal < 0 {
					return nil, besuderrors.Errorf(besuderrors.TransactionSendInputTypeBadByteOutsideRange, methodName, path, requiredType)
				}
			...
			return nil, besuderrors.Errorf(besuderrors.TransactionSendInputTypeBadJSONTypeForBytes, methodName, path, requiredType, suppliedType)
		}
		if len(bSlice) == 0 {
			return [0]byte{}, nil
	...
	case abi.TupleTy:
		if suppliedType.Kind() != reflect.Map || suppliedType.Key().Kind() != reflect.String {
			return nil, besuderrors.Errorf(besuderrors.TransactionSendInputTypeBadJSONTypeForTuple, methodName, path, requiredType, suppliedType)
		}
		return tx.generateTupleFromMap(methodName, path, requiredType, param.(map[string]interface{}))
	default:
		return nil, besuderrors.Errorf(besuderrors.TransactionSendInputTypeNotSupported, requiredType)
	}
}
...
func (tx *Txn) generateTypedArgs(origParams []interface{}, method *abi.Method) ([]interface{}, error) {
	...
	for idx, inputArg := range method.Inputs {
		if idx >= len(params) {
			err = besuderrors.Errorf(besuderrors.TransactionSendInputCountMismatch, methodName, len(method.Inputs), len(params))
			return nil, err
		}
		param := params[idx]
...
func flattenParams(origParams []interface{}, inputs *abi.Arguments, lazyTyping bool) (params []interface{}, err error) {
	if !lazyTyping && len(origParams) > len(*inputs) {
		err = besuderrors.Errorf(besuderrors.TransactionSendInputTooManyParams, len(origParams), len(*inputs))
	}
	...
	for i, unflattened := range origParams {
		if unflattened == nil {
			...
			if !exists {
				err = besuderrors.Errorf(besuderrors.TransactionSendInputStructureWrong, i)
				return
			}
			if reflect.TypeOf(typeStr).Kind() != reflect.String {
				err = besuderrors.Errorf(besuderrors.TransactionSendInputInLineTypeArrayNotString, i)
				return
			}
			params[i] = value
			// Set the type
			...
			if ethType, err = abi.NewType(typeStr.(string), "", []abi.ArgumentMarshaling{}); err != nil {
				err = besuderrors.Errorf(besuderrors.TransactionSendInputInLineTypeUnknown, i, typeStr, err)
				return
			}
			for len(*inputs) <= i {
