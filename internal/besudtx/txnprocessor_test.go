...
type testTxnContext struct {
	jsonMsg      string
	badMsgType   string
	replies      []besudmessages.ReplyWithHeaders
...
type testRPC struct {
	...
	ethGetTransactionReceiptResult besudeth.TxnReceipt
	ethGetTransactionReceiptErr    error
	privFindPrivacyGroupResult     []besudeth.OrionPrivacyGroup
...
func (r *testRPC) CallContext(ctx context.Context, result interface{}, method string, args ...interface{}) error {
	...
	if method == "eth_sendTransaction" || method == "eea_sendTransaction" {
		r.condLock.Lock()
		sendTX := args[0].(*besudeth.SendTXArgs)
...
func (c *testTxnContext) Headers() *besudmessages.CommonHeaders {
	commonMsg := besudmessages.RequestCommon{}
	if c.badMsgType != "" {
...
func (c *testTxnContext) Reply(replyMsg besudmessages.ReplyWithHeaders) {
...
func TestOnMessageBadMessage(t *testing.T) {
	...
	txnProcessor := NewTxnProcessor(&TxnProcessorConf{}, &besudeth.RPCConf{}).(*txnProcessor)
...
func TestOnDeployContractMessageBadJSON(t *testing.T) {
	...
	testTxnContext.badMsgType = besudmessages.MsgTypeDeployContract
	txnProcessor.OnMessage(testTxnContext)
...
func TestOnSendTransactionMessageBadJSON(t *testing.T) {
	...
	testTxnContext := &testTxnContext{}
	testTxnContext.jsonMsg = "badness"
	testTxnContext.badMsgType = besudmessages.MsgTypeSendTransaction
	txnProcessor.OnMessage(testTxnContext)
	for len(testTxnContext.errorReplies) == 0 {
		time.Sleep(1 * time.Millisecond)
	}
