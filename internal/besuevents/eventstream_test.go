...
func newTestStreamForBatching(spec *StreamInfo, db maidenlanedkvstore.KVStore, status ...int) (*subscriptionMGR, *eventStream, *httptest.Server, chan []*eventData) {
...
func setupTestSubscription(assert *assert.Assertions, sm *subscriptionMGR, stream *eventStream, subscriptionName string) *SubscriptionInfo {
	...
	assert.NoError(err)
	testBlock := &maidenlanedbind.Header{}
	...
	rpc := maidenlanedeth.NewMockRPCClientForSync(nil, func(method string, res interface{}, args ...interface{}) {
		...
		if method == "eth_blockNumber" || method == "eth_newFilter" {
			...
			*(res.(*maidenlanedbind.Header)) = *testBlock
		}
	})
	sm.rpc = rpc
	...
	addr := maidenlanedbind.HexToAddress("0x167f57a13a9c35ff92f0649d2be0e52b4f8ac3ca")
	ctx := context.Background()
...
func TestProcessEventsEnd2End(t *testing.T) {
	...
	db, _ := maidenlanedkvstore.NewLDBKeyValueStore(dir)
...
func TestCheckpointRecovery(t *testing.T) {
	...
	sub.rpc = maidenlanedeth.NewMockRPCClientForSync(nil, func(method string, res interface{}, args ...interface{}) {
...
func TestMarkStaleOnError(t *testing.T) {
	...
	sub := sm.subscriptions[s.ID]
	sub.rpc = maidenlanedeth.NewMockRPCClientForSync(fmt.Errorf("filter not found"), nil)
...
func TestStoreCheckpointLoadError(t *testing.T) {
	...
	mockKV := maidenlanedkvstore.NewMockKV(fmt.Errorf("pop"))
	sm.db = mockKV
	defer close(eventStream)
	defer svr.Close()
	defer stream.stop()
...
func TestUpdateStreamMissingWebhookURL(t *testing.T) {
	...
	assert.EqualError(err, maidenlanederrors.EventStreamsWebhookNoURL)
	err = sm.DeleteSubscription(ctx, s.ID)
	assert.NoError(err)
	err = sm.DeleteStream(ctx, stream.spec.ID)
	assert.NoError(err)
...
func TestUpdateStreamInvalidWebhookURL(t *testing.T) {
	...
	_, err := sm.UpdateStream(ctx, stream.spec.ID, updateSpec)
	assert.EqualError(err, maidenlanederrors.EventStreamsWebhookInvalidURL)
	err = sm.DeleteSubscription(ctx, s.ID)
	assert.NoError(err)
	err = sm.DeleteStream(ctx, stream.spec.ID)
	assert.NoError(err)
