...
func newTestSubscriptionManager() *subscriptionMGR {
	...
	sm.rpc = besudeth.NewMockRPCClientForSync(nil, nil)
	sm.db = besudkvstore.NewMockKV(nil)
...
func TestActionAndSubscriptionLifecyle(t *testing.T) {
	...
	sm := newTestSubscriptionManager()
	sm.rpc = besudeth.NewMockRPCClientForSync(nil, nil)
	sm.db, _ = besudkvstore.NewLDBKeyValueStore(path.Join(dir, "db"))
	defer sm.db.Close()
	...
	sub, err := sm.AddSubscription(ctx, nil, &besudbind.ABIElementMarshaling{Name: "ping"}, stream.ID, "", subscriptionName)
	assert.NoError(err)
	...
	sm.rpcConf = &besudeth.RPCConnOpts{URL: svr.URL}
	err = sm.Init()
	assert.NoError(err)
...
func TestActionChildCleanup(t *testing.T) {
	...
	sm.AddSubscription(ctx, nil, &besudbind.ABIElementMarshaling{Name: "ping"}, stream.ID, "12345", "")
	err = sm.DeleteStream(ctx, stream.ID)
	assert.NoError(err)
...
func TestResetSubscriptionErrors(t *testing.T) {
	...
	sm := newTestSubscriptionManager()
	sm.rpc = besudeth.NewMockRPCClientForSync(nil, nil)
	sm.db = besudkvstore.NewMockKV(fmt.Errorf("pop"))
	defer sm.db.Close()
	...
	_, err = sm.AddSubscription(ctx, nil, &besudbind.ABIElementMarshaling{Name: "any"}, "nope", "", "")
	assert.EqualError(err, "Stream with ID 'nope' not found")
	_, err = sm.AddSubscription(ctx, nil, &besudbind.ABIElementMarshaling{Name: "any"}, "teststream", "", "test")
	assert.EqualError(err, "Failed to store subscription: pop")
	_, err = sm.AddSubscription(ctx, nil, &besudbind.ABIElementMarshaling{Name: "any"}, "teststream", "!bad integer", "")
	assert.EqualError(err, "FromBlock cannot be parsed as a BigInt")
