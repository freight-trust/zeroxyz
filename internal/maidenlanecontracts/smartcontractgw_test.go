...
func TestNewSmartContractGatewayWithEvents(t *testing.T) {
	...
	s, err := NewSmartContractGateway(
		&SmartContractGatewayConf{
			...
			SubscriptionManagerConf: maidenlanedevents.SubscriptionManagerConf{
...
func TestPreDeployCompileAndPostDeploy(t *testing.T) {
	...
	assert.NoError(err)
	var deployStash maidenlanedmessages.DeployContract
	...
	router.ServeHTTP(res, req)
	assert.Equal(200, res.Result().StatusCode)
	var abi maidenlanedbind.RuntimeABI
...
func TestRegisterExistingContract(t *testing.T) {
	...
	req = httptest.NewRequest("POST", "/abis/"+abi.ID+"/0x0123456789abcdef0123456789abcdef01234567?maidenlaned-register=testcontract", bytes.NewReader([]byte{}))
	res = httptest.NewRecorder()
	router.ServeHTTP(res, req)
	...
	assert.Equal("testcontract", returnedSwagger.Info.Extensions["x-maidenlane-registered-name"])
...
func TestRemoteRegistrySwaggerOrABI(t *testing.T) {
	...
	res = httptest.NewRecorder()
	router.ServeHTTP(res, req)
	var returnedABI maidenlanedbind.RuntimeABI
	assert.Equal(200, res.Code)
...
func TestRemoteRegistryBadBI(t *testing.T) {
	...
	iMsg.ABI = append(iMsg.ABI, maidenlanedbind.ABIElementMarshaling{
		Type: "fallback",
	})
	iMsg.ABI = append(iMsg.ABI, maidenlanedbind.ABIElementMarshaling{
		Type: "fallback",
...
func TestLoadDeployMsgOKNoABIInIndex(t *testing.T) {
	...
	goodMsg := &maidenlanedmessages.DeployContract{}
...
func TestPostDeployNoRegisteredName(t *testing.T) {
	...
	contractAddr := common.HexToAddress("0x0123456789AbcdeF0123456789abCdef01234567")
	scgw := s.(*smartContractGW)
	replyMsg := &maidenlanedmessages.TransactionReceipt{
		ReplyCommon: maidenlanedmessages.ReplyCommon{
			Headers: maidenlanedmessages.ReplyHeaders{
				CommonHeaders: maidenlanedmessages.CommonHeaders{
					MsgType: maidenlanedmessages.MsgTypeTransactionSuccess,
...
func TestBuildIndex(t *testing.T) {
	...
	okSwagger.Info.AddExtension("x-maidenlane-deployment-id", "840b629f-2e46-413b-9671-553a886ca7bb")
	...
	regSwagger.Info.AddExtension("x-maidenlane-deployment-id", "840b629f-2e46-413b-9671-553a886ca7bb")
	regSwagger.Info.AddExtension("x-maidenlane-registered-name", "migratedcontract")
...
func TestStoreDeployableABIMissingABI(t *testing.T) {
	...
	_, err := scgw.storeDeployableABI(&maidenlanedmessages.DeployContract{}, nil)
...
func TestAddStreamNoSubMgr(t *testing.T) {
	assert := assert.New(t)
	res := testGWPath("POST", maidenlanedevents.StreamPathPrefix, nil, nil)
	assert.Equal(405, res.Result().StatusCode)
}

func TestAddStreamOK(t *testing.T) {
	assert := assert.New(t)
	spec := &maidenlanedevents.StreamInfo{Type: "webhook", Name: "stream-1", Timestamps: true}
	b, _ := json.Marshal(spec)
	req := httptest.NewRequest("POST", maidenlanedevents.StreamPathPrefix, bytes.NewReader(b))
	res := httptest.NewRecorder()
	s := &smartContractGW{}
	s.sm = &mockSubMgr{}
	r := &httprouter.Router{}
	s.AddRoutes(r)
	r.ServeHTTP(res, req)
	var newSpec maidenlanedevents.StreamInfo
...
func TestAddStreamDefaultNoTimestamps(t *testing.T) {
	assert := assert.New(t)
	spec := &maidenlanedevents.StreamInfo{Type: "webhook", Name: "stream-no-timestamps"}
	b, _ := json.Marshal(spec)
	req := httptest.NewRequest("POST", maidenlanedevents.StreamPathPrefix, bytes.NewReader(b))
	res := httptest.NewRecorder()
	s := &smartContractGW{}
...
func TestAddStreamBadData(t *testing.T) {
	assert := assert.New(t)
	req := httptest.NewRequest("POST", maidenlanedevents.StreamPathPrefix, bytes.NewReader([]byte(":bad json")))
	res := httptest.NewRecorder()
	s := &smartContractGW{}
	s.sm = &mockSubMgr{}
	r := &httprouter.Router{}
...
func TestAddStreamSubMgrError(t *testing.T) {
	assert := assert.New(t)
	spec := &maidenlanedevents.StreamInfo{Type: "webhook"}
	b, _ := json.Marshal(spec)
	req := httptest.NewRequest("POST", maidenlanedevents.StreamPathPrefix, bytes.NewReader(b))
	res := httptest.NewRecorder()
	s := &smartContractGW{}
...
func TestUpdateStreamNoSubMgr(t *testing.T) {
	assert := assert.New(t)
	res := testGWPath("PATCH", maidenlanedevents.StreamPathPrefix+"/123", nil, nil)
	assert.Equal(405, res.Result().StatusCode)
}

func TestUpdateStreamOK(t *testing.T) {
	assert := assert.New(t)
	spec := &maidenlanedevents.StreamInfo{Type: "webhook", Name: "stream-new-name", ID: "123"}
	b, _ := json.Marshal(spec)
	req := httptest.NewRequest("PATCH", maidenlanedevents.StreamPathPrefix+"/123", bytes.NewReader(b))
	res := httptest.NewRecorder()
	s := &smartContractGW{}
	s.sm = &mockSubMgr{}
	r := &httprouter.Router{}
...
func TestUpdateStreamBadData(t *testing.T) {
	assert := assert.New(t)
	req := httptest.NewRequest("PATCH", maidenlanedevents.StreamPathPrefix+"/123", bytes.NewReader([]byte(":bad json")))
	res := httptest.NewRecorder()
	s := &smartContractGW{}
	s.sm = &mockSubMgr{}
	r := &httprouter.Router{}
...
func TestUpdateStreamNotFoundError(t *testing.T) {
	assert := assert.New(t)
	spec := &maidenlanedevents.StreamInfo{Type: "webhook", ID: "123"}
	b, _ := json.Marshal(spec)
	req := httptest.NewRequest("PATCH", maidenlanedevents.StreamPathPrefix+"/123", bytes.NewReader(b))
	res := httptest.NewRecorder()
	s := &smartContractGW{}
...
func TestListStreamsNoSubMgr(t *testing.T) {
	assert := assert.New(t)
	res := testGWPath("GET", maidenlanedevents.StreamPathPrefix, nil, nil)
	assert.Equal(405, res.Result().StatusCode)
}
...
func TestListStreams(t *testing.T) {
	...
	var results []*maidenlanedevents.StreamInfo
	res := testGWPath("GET", maidenlanedevents.StreamPathPrefix, &results, mockSubMgr)
	assert.Equal(200, res.Result().StatusCode)
...
func TestListSubs(t *testing.T) {
	...
	var results []*maidenlanedevents.SubscriptionInfo
	res := testGWPath("GET", maidenlanedevents.SubPathPrefix, &results, mockSubMgr)
	assert.Equal(200, res.Result().StatusCode)
	assert.Equal(2, len(results))
	assert.Equal("later", results[0].ID)
	assert.Equal("earlier", results[1].ID)
...
func TestGetSub(t *testing.T) {
	...
	mockSubMgr := &mockSubMgr{
		sub: &maidenlanedevents.SubscriptionInfo{ID: "123"},
	}
	var result maidenlanedevents.SubscriptionInfo
	res := testGWPath("GET", maidenlanedevents.SubPathPrefix+"/123", &result, mockSubMgr)
	assert.Equal(200, res.Result().StatusCode)
...
func TestGetStream(t *testing.T) {
	...
	mockSubMgr := &mockSubMgr{
		stream: &maidenlanedevents.StreamInfo{ID: "123"},
	}
	var result maidenlanedevents.StreamInfo
	res := testGWPath("GET", maidenlanedevents.StreamPathPrefix+"/123", &result, mockSubMgr)
	assert.Equal(200, res.Result().StatusCode)
	assert.Equal("123", result.ID)
}
...
func TestGetSubNoSubMgr(t *testing.T) {
	...
	var result maidenlanedevents.SubscriptionInfo
	res := testGWPath("GET", maidenlanedevents.SubPathPrefix+"/123", &result, nil)
	assert.Equal(405, res.Result().StatusCode)
}
...
func TestDeleteSub(t *testing.T) {
	...
	mockSubMgr := &mockSubMgr{}
	res := testGWPath("DELETE", maidenlanedevents.SubPathPrefix+"/123", nil, mockSubMgr)
...
func TestResetSub(t *testing.T) {
	...
	mockSubMgr := &mockSubMgr{}
	res := testGWPathBody("POST", maidenlanedevents.SubPathPrefix+"/123/reset", nil, mockSubMgr, bytes.NewReader(b))
	assert.Equal(204, res.Result().StatusCode)
}
...
func TestResetSubNoManager(t *testing.T) {
	assert := assert.New(t)
	res := testGWPath("POST", maidenlanedevents.SubPathPrefix+"/123/reset", nil, nil)
	assert.Equal(405, res.Result().StatusCode)
}
...
func TestDeleteStream(t *testing.T) {
	...
	mockSubMgr := &mockSubMgr{}
	res := testGWPath("DELETE", maidenlanedevents.StreamPathPrefix+"/123", nil, mockSubMgr)
	assert.Equal(204, res.Result().StatusCode)
}
...
func TestDeleteSubNoSubMgr(t *testing.T) {
	...
	res := testGWPath("DELETE", maidenlanedevents.SubPathPrefix+"/123", nil, nil)
	assert.Equal(405, res.Result().StatusCode)
}
...
func TestDeleteSubError(t *testing.T) {
	...
	res := testGWPath("DELETE", maidenlanedevents.SubPathPrefix+"/123", &errInfo, mockSubMgr)
	assert.Equal(500, res.Result().StatusCode)
...
func TestSuspendStream(t *testing.T) {
	...
	mockSubMgr := &mockSubMgr{}
	res := testGWPath("POST", maidenlanedevents.StreamPathPrefix+"/123/suspend", nil, mockSubMgr)
	assert.Equal(204, res.Result().StatusCode)
...
func TestResumeStream(t *testing.T) {
	...
	mockSubMgr := &mockSubMgr{}
	res := testGWPath("POST", maidenlanedevents.StreamPathPrefix+"/123/resume", nil, mockSubMgr)
	assert.Equal(204, res.Result().StatusCode)
...
func TestResumeStreamFail(t *testing.T) {
	...
	var errInfo = restErrMsg{}
	res := testGWPath("POST", maidenlanedevents.StreamPathPrefix+"/123/resume", &errInfo, mockSubMgr)
	assert.Equal(500, res.Result().StatusCode)
...
func TestSuspendNoSubMgr(t *testing.T) {
	...
	res := testGWPath("POST", maidenlanedevents.StreamPathPrefix+"/123/resume", nil, nil)
	assert.Equal(405, res.Result().StatusCode)
}
