...
type testKafkaCommon struct {
	...
	kafkaFactory    *maidenlanedkafka.MockKafkaFactory
	kafkaInitDelay  int
	startTime       time.Time
}
...
func (k *testKafkaCommon) Conf() *maidenlanedkafka.KafkaCommonConf {
	return &maidenlanedkafka.KafkaCommonConf{}
}

func (k *testKafkaCommon) Producer() maidenlanedkafka.KafkaProducer {
	var producer maidenlanedkafka.KafkaProducer
...
func newTestKafkaComon() *testKafkaCommon {
	...
	kafka.kafkaFactory = maidenlanedkafka.NewMockKafkaFactory()
...
func newTestWebhooks() (*webhooks, *webhooksKafka, *testKafkaCommon, *httptest.Server) {
	...
	ts.Config.BaseContext = func(l net.Listener) context.Context {
		ctx, _ := maidenlanedauth.WithAuthContext(context.Background(), "testat")
		return ctx
	}
	ts.Start()
	return w, wk, k, ts
...
func assertSentResp(assert *assert.Assertions, resp *http.Response, ack bool) {
	...
	if resp.StatusCode == 200 {
		var replyMsg maidenlanedmessages.AsyncSentMsg
...
func TestWebhookHandlerJSONSendTransaction(t *testing.T) {
	...
	forwardedMessage := maidenlanedmessages.SendTransaction{}
	json.Unmarshal(replyMsgs[0], &forwardedMessage)
	assert.Equal(maidenlanedmessages.MsgTypeSendTransaction, forwardedMessage.Headers.MsgType)
...
func TestWebhookHandlerJSONDeployContract(t *testing.T) {
	...
	msg.Headers.MsgType = maidenlanedmessages.MsgTypeDeployContract
	msg.From = "any string"
	msgBytes, _ := json.Marshal(&msg)
	resp, replyMsgs := sendTestTransaction(assert, msgBytes, "application/json", nil, true)
	assertSentResp(assert, resp, true)
	...
	forwardedMessage := maidenlanedmessages.SendTransaction{}
	json.Unmarshal(replyMsgs[0], &forwardedMessage)
	assert.Equal(maidenlanedmessages.MsgTypeDeployContract, forwardedMessage.Headers.MsgType)
}
...
func TestWebhookHandlerBadMsgType(t *testing.T) {
	...
	msg := maidenlanedmessages.RequestCommon{}
...
func TestConsumerMessagesLoopCallsReplyProcessorWithEmptyPayload(t *testing.T) {
	...
	consumer.(*maidenlanedkafka.MockKafkaConsumer).MockMessages <- &sarama.ConsumerMessage{
		Partition: 3,
		Offset:    12345,
		Value:     []byte(""),
	}
	...
	assert.Equal(int64(12345), consumer.(*maidenlanedkafka.MockKafkaConsumer).OffsetsByPartition[3])
